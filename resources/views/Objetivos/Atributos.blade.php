...
@section('content')
            ...
            <div class="row">
                <div class="col-lg-12">
                    <div class="card shadow p-3 mb-5 bg-body rounded">
                        @if ($envio->count() == 0)
                            <h1 class="text-center">No Existen Atributos</h1>
                        @else
                            <div class="card-body">
                                <div class="row">
                                    <table class="tabla-general">
                                        ...
                                        <tbody>
                                            @foreach ($envio as $atributo)
                                                <tr class="table100-head">
                                                    <td class="column1">{{ $loop->iteration }}</td>
                                                    <td class="column2">{{ $atributo->descripcion }}</td>
                                                    <td class="column5">{{ $atributo->meta }}</td>
                                                    <td class="column4">
                                                        <div>
                                                            <button class="chip primary" data-toggle="modal"
                                                                data-target="#modalAspectosAtributos{{ $atributo->id }}">
                                                                <span>
                                                                    Aspectos: {{ $atributo->aspectos->count() }}
                                                                </span>
                                                            </button>
                                                        </div>
                                                    </td>
                                                    <td class="column3">
                                                        <div class="acciones">
                                                            @can('editar-atributos')
                                                                <button class="btn-tabla" data-toggle="modal"
                                                                    data-target="#modalEditar{{ $atributo->id }}">
                                                            ...
                                                            @endcan
                                                            @can('borrar-atributos')
                                                                <form action="{{ url('Atributos', [$atributo->id]) }}"
                                                            ...
                                                            @endcan
                                                            @can('ver-aspectosAtributos')
                                                                <form
                                                                    action="{{ route('AspectosAtributos.show', $atributo->id) }}"
                                                                    method="GET">
                                                        ...
                                                        {{-- <button class="btn btn-primary btn-md" data-toggle="modal"
                                                            data-target="#modalEditar{{ $atributo->id }}">Editar</button>
                                                        <form method="POST"
                                                            action="{{ url('Atributos/' . $atributo->idAtributo) }}">
                                                            @csrf
                                                            {{ method_field('DELETE') }}
                                                            <input type="submit"
                    ...
                    @endif
                    @can('crear-atributos')
                        <a href="#" class="btn-flotante" data-toggle="modal" data-target="#modalAgregar">Agregar Atributo</a>
                    @endcan
                </div>
            </div>
        </div>
    ...
    @foreach ($envio as $atributo)
        <div class="modal fade" id="modalEditar{{ $atributo->id }}" tabindex="-1" aria-labelledby="exampleModalLabel"
            aria-hidden="true">
            <div class="modal-dialog">
                <div class="modal-content">
                    <div class="modal-header">
                    ...
                    <form action="{{ url('Atributos/' . $atributo->id) }}" method="POST">
                        @method('PATCH')
                        @csrf
                        <div class="modal-body">
                            ...
                            <textarea type="text" class="form-control" name="descripcion" rows="5" style="resize: none; height: 6rem;"
                                autofocus required>{{ $atributo->descripcion }}</textarea>
                                <label for="carrera">Meta</label>
                                <input type="number" class="form-control" name="meta" max="5" min="0" step="0.1" value="{{$atributo->meta}}">
                            <input type="text" class="form-control" name="idCarrera"
        ...
        <!-- Modal para consultar los aspectos de un atributo -->
        <div class="modal fade" id="modalAspectosAtributos{{ $atributo->id }}">
            <div class="modal-dialog modal-lg" style="z-index: 4000;">
                <div class="modal-content">
                    <div class="modal-header">
                        <h5 class="modal-title" id="exampleModalLabel">Aspectos de: {{ $atributo->descripcion }}</h5>
                        <button class="btn-tabla" type="button" data-dismiss="modal">
                            <div class="icon trash-fill">
                                <i>
                                    <svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 320 512">
                    ...
                    <div class="modal-body">
                        ...
                        <table>
                            ...
                            <tbody>
                                @foreach ($atributo->aspectos as $aspecto)
                                    <tr>
                                        <td class="column1-modal"">{{ $loop->iteration }}</td>
                                    <td class=" column2-modal-objetivos-atributos">{{ $aspecto->nombre }}</td>
                                    </tr>
                                @endforeach
                            </tbody>
                        </table>
